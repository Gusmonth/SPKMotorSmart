...
if ($cek > 0) {
	header("Location: hitung.php?validasi=minus");
} else {
	...
	if ($total_bobot > 100 || $total_bobot < 100) {
		header("Location: hitung.php?validasi=warning");
	} else {
		$array_nilai = array();
		$i = 0;
		...
		while ($data_checked = mysqli_fetch_array($query_checked)) {
			...
			$j = 0;
			$query_matriks = mysqli_query($koneksi, "SELECT subkriteria.nilai_subkriteria AS nilai FROM matriks JOIN subkriteria ON matriks.id_subkriteria = subkriteria.id_subkriteria JOIN checked ON matriks.id_alternatif = checked.id_alternatif WHERE checked.id_alternatif = '$id_alternatif'");
		...
		$id_alternatif = array();
		$query_checked = mysqli_query($koneksi, "SELECT alternatif.id_alternatif AS id_alternatif FROM alternatif JOIN checked ON alternatif.id_alternatif = checked.id_alternatif");
		while ($data_checked = mysqli_fetch_array($query_checked)) {
			$id_alternatif[] = $data_checked['id_alternatif'];
		}
		...
		if (mysqli_num_rows($cek) > 0) {
			$kosongkan = mysqli_query($koneksi, "DELETE FROM peringkat");
			if ($kosongkan) {
				for ($i=0; $i < count($id_alternatif); $i++) { 
					$id = $id_alternatif[$i];
					$nilai = $nilai_akhir[$i];

					$insert = mysqli_query($koneksi, "INSERT INTO peringkat(id_alternatif, nilai_peringkat) VALUES('$id', '$nilai')");
					if ($insert) {
						header("Location: hasil-index.php?validasi=sukses");
					} else {
